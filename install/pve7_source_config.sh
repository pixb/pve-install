...
cat <<<_EOF_
"${COLOR_GREEN}GREEN${COLOR_NC}"
"${COLOR_YELLOW}YELLOW${COLOR_NC}"
"${COLOR_RED}RED${COLOR_NC}"
_EOF_
...
# download gpg
wget https://mirrors.ustc.edu.cn/proxmox/debian/proxmox-release-bullseye.gpg -O /etc/apt/trusted.gpg.d/proxmox-release-bookworm.gpg

APT_SOURCES_DIR="/etc/apt/sources.list.d"
USTC_CEPH_SOURCE="deb https://mirrors.ustc.edu.cn/proxmox/debian/ceph-pacific bullseye main"
USTC_ENTERPRISE_SOURCE="deb https://mirrors.tuna.tsinghua.edu.cn/proxmox/debian bullseye pve-no-subscription"

# ceph source
if [ ! -e ${APT_SOURCES_DIR}/ceph.list.bak ]; then
  echo -e "${COLOR_YELLOW}ceph.list backup file don't exists. create it.${COLOR_NC}"
...
if [ ! -e ${APT_SOURCES_DIR}/pve-enterprise.list.bak ]; then
  echo -e "${COLOR_YELLOW}pve-enterprise.list backup file don't exists. create it.${COLOR_NC}"
